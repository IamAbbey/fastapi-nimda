...
class AdminForm:
    ...
    def get_columns_widget(self, columns: List[Column]) -> Dict[str, Widget]:
                ...
                if (
                    ...
                    if (
                        ...
                        with Session(self.engine) as session:
                            ...
                            c_model_pk = get_any_model_primary_keys(
                                column.mapper.class_
                            )[0]
                            choices = [
        ...
        return templates.get_template("form/div.html").render(
            {
                "fields": {
                    name: {
                        "label": widget.render_label(name=name, value=None),
                        "field": widget.render(
                            name=name, value=widget.attrs.get("value")
                        ),
                        "error": name in kwargs.get("error_fields", []),
                    }
                    for name, widget in self.get_render_widgets().items()
    ...
    def validate_form(self, **kwargs):
        ...
        for name, widget in self.get_render_widgets().items():
            if name not in exclude:
                response[name] = widget.format_value(form_body.get(name))
        return response
